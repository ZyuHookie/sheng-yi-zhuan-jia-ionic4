...
```typescript
async login(username: string, password: string): Promise<AjaxResult> {
...
>
> 要了解更多的Toast Controller的知识，请参考[Ionic官网](https://ionicframework.com/docs/api/toast-controller)
...
>
> 要了解更多的Alert Controller的知识，请参考[Ionic官网](https://ionicframework.com/docs/api/alert-controller)
...
4. 在欢迎页组件中调整ionViewWillEnter代码或者路由守护的代码，判断用户是否已登录，已登录过跳转到首页。
...
04. 在登录页中使用CopyrightComponent代替之前的div。使用时需要导入之前创建的模块，在imports属性中添加CzbModule。
